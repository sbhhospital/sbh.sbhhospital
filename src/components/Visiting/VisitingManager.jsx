            ...
            <div className="bg-white border-b border-slate-200 -mx-6 px-10 py-3 sticky top-0 z-30 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm">
                <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1 md:pb-0">
                    {[
                        ...
                        { id: 'SETTLED', label: 'History', icon: Clock },
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-none shrink-0 ${
                                activeSubTab === tab.id 
                    ...
                    <button onClick={fetchData} className="p-2 bg-slate-50 border border-slate-100 rounded-lg text-slate-400 hover:text-orange-600 transition-all active:scale-95 shadow-sm"><RefreshCw size={12} /></button>
                </div>
                ...
                <div className="relative flex-1 max-w-md mx-4 group">
                    ...
                    <input 
                        ...
                        className="w-full bg-slate-50 border-2 border-transparent rounded-2xl py-2 pl-12 pr-4 text-[11px] font-black text-slate-800 outline-none focus:bg-white focus:border-orange-500/20 focus:ring-4 focus:ring-orange-500/5 transition-all shadow-inner"
                    />
                            ...
                            {filteredSearchResults.map((res, i) => (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${res.Status === 'Paid' ? 'bg-orange-50 text-orange-600' : 'bg-orange-50 text-orange-600'}`}>
                                            <User size={14} />
                                        </div>
                ...
                <div className="hidden md:flex items-center gap-3">
                    <div className="flex items-center gap-2 text-[8px] font-black text-slate-400 uppercase tracking-widest">
                        <span className="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse"></span> Service: Ledger-v4.1
                    </div>
                </div>
            </div>
            ...
            {activeSubTab === 'DASHBOARD' && (
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                        ...
                        <div className="bg-white rounded-xl p-3 border border-slate-100 shadow-sm flex items-center gap-3 hover:border-orange-200 transition-all">
                            <div className="w-8 h-8 rounded-lg bg-orange-50 flex items-center justify-center text-orange-600"><IndianRupee size={16} /></div>
                            <div>
            ...
            {activeSubTab === 'HR_ENTRY' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-sm h-fit">
                        <h3 className="text-sm font-black text-slate-800 uppercase mb-8 flex items-center gap-3"><IndianRupee className="text-orange-500" size={18} /> Log Professional Visit</h3>
                        <form onSubmit={handleLogPayment} className="space-y-6">
                            <div className="space-y-2">
                                ...
                                <select 
                                    ...
                                    className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-[11px] font-black focus:bg-white focus:border-orange-500/30 outline-none uppercase transition-all shadow-sm"
                                >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input required type="number" value={paymentFormData.amountPerVisit} onChange={e => setPaymentFormData({...paymentFormData, amountPerVisit: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 pl-8 text-[12px] font-black focus:bg-white focus:border-orange-500/30 outline-none transition-all shadow-sm" placeholder="0" />
                                    </div>
                                </div>
                                <div className="space-y-2">
                             ...
                             <div className="space-y-4 pt-4 border-t border-slate-100">
                                ...
                                <div className="flex gap-3">
                                    <input type="date" value={paymentFormData.currentDate} onChange={e => setPaymentFormData({...paymentFormData, currentDate: e.target.value})} className="flex-1 bg-slate-50 border border-slate-200 rounded-xl p-3 text-[11px] font-black focus:bg-white focus:border-orange-500/30 outline-none uppercase transition-all shadow-sm" />
                                    <button type="button" onClick={handleAddVisitDate} className="px-6 bg-orange-600 text-white rounded-xl flex items-center justify-center hover:bg-orange-700 transition-all shadow-md active:scale-95"><Plus size={16} /></button>
                                </div>
                                
                                <div className="flex flex-wrap gap-2 mt-2 min-h-[40px]">
                                    {paymentFormData.visitDates.map(date => (
                                        <div key={date} className="flex items-center gap-2 px-4 py-2 bg-slate-50 text-slate-600 rounded-xl text-[10px] font-black border border-slate-200 shadow-sm">
                                            {date}
                            ...
                            <div className="p-6 bg-slate-900 rounded-2xl flex flex-col gap-4 border-b-4 border-orange-500">
                                ...
                                <div className="flex justify-between items-center px-2">
                                    <p className="text-[9px] font-black text-orange-500 uppercase tracking-widest">Final Net Payable</p>
                            ...
                            <button disabled={submitting} type="submit" className="w-full py-4 bg-orange-600 text-white rounded-xl font-black uppercase tracking-widest text-[11px] hover:bg-orange-500 transition-all shadow-lg disabled:opacity-50 flex items-center justify-center gap-3">
            ...
            {activeSubTab === 'SETTLED' && (
                <div className="bg-white rounded-[2rem] border border-slate-200 overflow-hidden shadow-sm">
                    <div className="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            <h3 className="text-[11px] font-black text-slate-800 uppercase tracking-widest flex items-center gap-3"><CheckCircle2 className="text-orange-500" size={16} /> Current Month Settlements</h3>
                        ...
                        <div className="px-5 py-2 bg-orange-50 text-orange-600 rounded-full text-[9px] font-black uppercase tracking-widest border border-orange-100">
                    ...
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {settlementsThisMonth.map((p, i) => (
                            <div 
                                key={i} 
                                className="p-5 rounded-2xl bg-white border border-slate-100 hover:border-orange-500/30 hover:shadow-xl hover:shadow-orange-50 transition-all group"
                            >
                                <div className="flex items-center justify-between mb-4">
                                    <div className="w-9 h-9 rounded-xl bg-orange-50 flex items-center justify-center text-orange-600 group-hover:bg-orange-600 group-hover:text-white transition-all"><CheckCircle2 size={16} /></div>
                                ...
                                <div className="mt-4 pt-4 border-t border-slate-100 flex flex-col gap-3">
                                    <div className="flex items-center justify-between">
                                        <p className="text-[14px] font-black text-orange-600 tracking-tight">₹{parseFloat(p.Paid_Amount || p.Amount_To_Pay || 0).toLocaleString()}</p>
                                        <div className="text-right">
                ...
                <div className="bg-white rounded-[2rem] border border-slate-200 overflow-hidden shadow-sm">
                    <div className="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="flex items-center gap-4">
                            <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm text-orange-600 border border-orange-50"><Briefcase size={18} /></div>
                            <div>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-slate-50/50 border-b border-slate-100">
                                <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Entry ID</th>
                                <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Specialist</th>
                                <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Period</th>
                                <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Settlement</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                            {paginatedHistory.map((p, idx) => (
                                <tr key={idx} className="hover:bg-slate-50/50 transition-all group">
                                    <td className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-tighter">#{p.Payment_ID?.slice(-6)}</td>
                                    <td className="px-6 py-3">
                                        <p className="text-[10px] font-black text-slate-800 uppercase leading-none">{p.Doctor_Name}</p>
                                        <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-1">{p.Doctor_ID}</p>
                                    </td>
                                    <td className="px-6 py-3 text-[9px] font-bold text-slate-600">{p.Month} {p.Year}</td>
                                    <td className="px-6 py-3 text-right">
                                        <div className="flex flex-col items-end">
                                            <span className="text-[11px] font-black text-slate-900">₹{parseFloat(p.Net_Amount || p.Paid_Amount || 0).toLocaleString('en-IN')}</span>
                                            <span className={`text-[7px] font-black uppercase tracking-widest ${String(p.Status).toUpperCase() === 'SETTLED' || String(p.Status).toUpperCase() === 'PAID' ? 'text-orange-500' : 'text-orange-500'}`}>{p.Status}</span>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                            {filteredHistory.length === 0 && (
                                <tr><td colSpan="4" className="text-center py-20 text-[11px] font-black uppercase text-slate-300 italic tracking-widest">No matching records found.</td></tr>
                            )}
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div className="relative w-full max-w-md bg-white rounded-[3rem] p-10 shadow-2xl space-y-8">
                                ...
                                <div className="space-y-4 pt-2 border-t border-slate-200/50">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <input 
                                            ...
                                            className="w-full bg-white border border-slate-200 rounded-2xl p-4 text-[11px] font-black outline-none focus:border-orange-500 transition-all"
                                        />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-4">
                                ...
                                <button 
                                    ...
                                    className="flex-1 py-4 bg-orange-600 text-white rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl shadow-orange-600/20 active:scale-95 transition-all disabled:opacity-50"
                                >
                                    {submitting ? (
                    ...
                    >
                            <div className="p-8 md:p-10">
                                ...
                                <div className="text-center mb-6">
                                    <div className="w-16 h-16 bg-orange-50 text-orange-600 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-orange-50">
                                        <Award size={32} />
                                    </div>
                                ...
                                <div className="space-y-3 bg-slate-50 rounded-[2rem] p-6 border border-slate-100 mb-8">
                                    ...
                                    {selectedPayment.Status === 'Paid' && (
                                        <div className="flex items-center justify-between py-2.5 border-b border-slate-200/50">
                                            <p className="text-[8px] font-black text-orange-600 uppercase tracking-[0.2em]">Settled On</p>
                                            <p className="text-[10px] font-black text-orange-700">{selectedPayment.Payment_Date}</p>
                                        </div>
                                    )}
                                    <div className="flex items-center justify-between py-2.5 border-b border-slate-200/50">
                                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em]">Status</p>
                                        <span className={`px-3 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest ${selectedPayment.Status === 'Paid' ? 'bg-orange-500 text-white shadow-md' : 'bg-orange-100 text-orange-600'}`}>
                                    ...
                                    <div className="flex items-center justify-between py-2.5">
                                        ...
                                        <div className="flex items-center gap-2 text-[9px] font-black text-slate-600 uppercase italic">
                                            <div className={`w-1.5 h-1.5 rounded-full ${selectedPayment.Status === 'Paid' ? 'bg-orange-500' : 'bg-orange-500 animate-pulse'}`}></div>
                                            {selectedPayment.Status === 'Paid' ? 'Archive' : 'Accounting'}
                                        </div>
                                    </div>
                                </div>

                                <button onClick={() => setSelectedPayment(null)} className="w-full py-4 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-orange-600 transition-all shadow-xl flex items-center justify-center gap-3 group">
        ...
        <div className="space-y-4">
            ...
            {pending.length === 0 ? (
                ...
                pending.map((item, idx) => {
                    ...
                    return (
                        <div key={idx} className="bg-white rounded-[2rem] border border-slate-100 p-8 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-8 hover:border-orange-200 transition-all group">
                            <div className="flex items-center gap-6">
                                <div className="w-14 h-14 bg-slate-50 text-slate-400 rounded-2xl flex items-center justify-center font-black border border-slate-100 group-hover:bg-orange-50 group-hover:text-orange-600 transition-all">
                                    <Stethoscope size={24} />
                                </div>
                                <div>
                                    <h4 className="text-lg font-black text-slate-800 uppercase tracking-tighter leading-none mb-2">{item.Doctor_Name}</h4>
                                    <div className="flex items-center gap-3">
                                        <span className="text-[9px] font-black text-orange-600 uppercase tracking-widest">{doc?.Specialty || 'Consultant'}</span>
                            ...
                            <div className="flex-1 lg:px-10 space-y-4">
                                <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
                                    ...
                                    <div><p className="text-[8px] font-black text-orange-600 uppercase mb-1">Net Pay</p><p className="text-lg font-black text-orange-600 tracking-tighter">₹{(item.Net_Amount || item.Amount_To_Pay || 0).toLocaleString()}</p></div>
                                </div>
                                <input 
                                    type="text" 
                                    ...
                                    className="w-full bg-slate-50 border-none rounded-xl py-3 px-4 text-[10px] font-black outline-none focus:ring-2 focus:ring-orange-500/10 transition-all" 
                                />
                            </div>
                            ...
                            <button 
                                ...
                                disabled={submitting} 
                                className="px-8 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-3 shadow-xl hover:bg-orange-600 transition-all active:scale-95 disabled:opacity-50"
                            >
